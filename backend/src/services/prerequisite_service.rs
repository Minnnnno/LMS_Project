...
async fn validate_required_modules(
    ...
    if let Some(module_id) = target_module_id {
        if required_module_ids.contains(&module_id) {
            return Err(HttpResponse::BadRequest().body("A module cannot require itself"));
        }
    }
...
pub async fn replace_module_prerequisites(
    ...
    validate_required_modules(db, course_id, Some(module_id), &required_module_ids).await?;
    ...
    for required_module_id in required_module_ids {
        ...
        .map_err(|err| {
            HttpResponse::InternalServerError()
                .body(format!("Database error saving quiz prerequisite: {}", err))
        })?;
    }
